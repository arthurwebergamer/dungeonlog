...
47. [x] **Botão de idioma em Config, com bandeira (pedido do usuário, mesma sessão do item 46).** Linha nova "Idioma" em Perfil → Config, entre Aparência e Preferências — mesmo padrão visual `.cfgrow.cfglink` de Herói/Aparência/Preferências (item 39), abre uma sub-tela própria com a lista de opções.
    ...
    - **Resumo na tela principal de Config** (`cfgIdiomaResumo`): mostra bandeira + nome do idioma atual ("🇧🇷 Português" / "🇺🇸 English"), atualizado em `atualizarResumosConfig()` (mesma função que já atualiza os resumos de Herói/Aparência/Preferências).
    ...
    - **Validado**: `node --check` nos 42 blocos + `assets.js` (0 erros), balanço de `<div>` (367/367 — subiu de 359 por causa do overlay novo), `<svg>` (56/56), comentários (38/38). Todos os IDs novos (`cfgAbrirIdioma`, `idiomaOverlay`, `idiomaVoltarBtn`, `idiomaGrid`, `cfgIdiomaResumo`) conferidos por script — nenhum `getElementById` órfão. Dicionário `TRADUCOES` re-testado (111 chaves agora, zero duplicata, zero chave órfã). **Teste funcional com jsdom real**: simulei o clique na opção "English" e confirmei que `definirIdioma('en')` é chamado e o check visual muda pra opção certa — não só lido, executado. **Não testado em aparelho real.**
