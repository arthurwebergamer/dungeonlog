...
46. [~] **Botões de idioma/tema saem do login, idioma ganha slide próprio no carrossel — i18n completo (UI + diálogos + itens/monstros) vira projeto de verdade, não fix rápido (levantado em sessão de brainstorm, 28/08).** *(⚠️ IMPLEMENTAÇÃO PARCIAL, mesma sessão de código — arquitetura + slide prontos, tradução do conteúdo NÃO — ver detalhe abaixo)* Item 13d, aberto desde o início desta sessão de brainstorm, finalmente decidido.
    - **O que foi implementado nesta sessão (`index.html`/`style.css`):**
      ...
      - **Módulo `I18N — IDIOMA` novo** (bloco autônomo, fim do arquivo): dicionário `TRADUCOES` (`{pt,en}` por chave), `window.t('chave')`, `window.aplicarIdioma()` (varre `[data-i18n]`) e `window.definirIdioma(lang)`. Aplicado nesta sessão só nos **4 labels da nav inferior** (Tarefas/Inventário/Loja/Perfil, via `data-i18n="nav.*"`) — prova de que o mecanismo funciona ponta a ponta, não é escopo completo.
      - CSS novo (`.carrosselIdiomaEscolha`/`.idiomaOpcao`) reaproveita tokens de tema existentes, sem cor nova.
    - **O que NÃO foi feito, de propósito, por causa do escopo (mesma decisão já registrada abaixo):** os ~79 pontos de texto cru do resto do app, os diálogos (item 12), e os campos `nome_en` em `ITENS`/`MONSTROS` — nada disso foi tocado. `window.t()`/`data-i18n` já existem prontos pra receber isso incrementalmente, sessão por sessão, sem precisar mexer na arquitetura de novo.
    - **Validado:** `node --check` nos 42 blocos `<script>` (40 clássicos + 2 `module`), balanço de `<div>` (359/359), `<svg>` (52/52), `<!--`/`-->` (36/36). **Não testado em aparelho real** — mesma pendência de sempre nesta fase.
