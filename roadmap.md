...
39. [ ] **Config reestruturado: tela principal vira lista enxuta (texto + seta), conteúdo pesado migra pra sub-telas (levantado em sessão de brainstorm, 26/08).** Usuário trouxe 3 prints de referência (QuestLog atual, Habitica, e um terceiro app) — achado do terceiro: header com avatar+nome+email editáveis inline, e todo o resto em lista categorizada (`Idioma >`, `Aparência >`, cada linha abrindo uma sub-tela própria), não tudo exposto cru na mesma tela. Comparação completa das 3 referências ficou registrada nesta sessão, mas **decisão final do usuário foi mais estreita que o leque oferecido**: não quis email editável, não quis pausar dano, não quis hora de reinicialização editável — só a reestruturação visual (densidade de informação), nada de feature nova.
    - **Diagnóstico:** Config hoje mostra tudo cru na tela principal — grade de 8 sprites de herói e 5 bolinhas de cor de tema, lado a lado com Conta/Apagar dados. Comparado à referência (só texto + valor atual + seta, conteúdo pesado numa tela própria), fica denso demais.
    - **Estrutura aprovada:**
      - **Tela principal (Config), enxuta:** `CONTA` (Conta, Apagar dados — sem mudança, já são só 2 linhas) + `APP` (3 linhas novas: "Herói >" mostrando o nome atual, "Aparência >" mostrando o nome do tema atual, "Preferências >" mostrando vibrar/som atual).
      - **3 sub-telas novas**, mesmo padrão de overlay que o próprio Config já usa desde a v4.58 (seção 42): **Herói** (nome do herói + grade de 8 sprites, migrado tal como já existe), **Aparência** (5 bolinhas de tema, migrado tal como já existe), **Preferências** (feedback de clique vibrar/som, migrado tal como já existe).
      - **Zero lógica nova, zero CSS de conteúdo novo** — é mover o que já existe pra outro container, reaproveitando o mecanismo de overlay já validado. O trabalho real é: criar as 3 sub-telas vazias com o padrão visual certo, mover o markup dos 3 blocos pra dentro delas, e trocar a tela principal pelas 3 linhas resumo com seta.
    - Ainda não iniciado. Não bloqueia lançamento.
...
3. **`#mobSprite` na arena** ainda tem a silhueta de monstro roxo
   estática (ver acima) — perguntar se entra no escopo da limpeza.
...
2. `#mobSprite` da arena ainda tem a silhueta de monstro roxo estática
   -- perguntar se entra no escopo de alguma limpeza futura.
